<?php

namespace App\Http\Controllers;

use App\Http\Middleware\AsyncTask;
use Exception;
use Illuminate\Database\Eloquent\Casts\Json;
use Illuminate\Support\Facades\Cache;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Str;

class ApiController extends Controller
{
    public static function getAccountInfo($cookie)
    {
        $accountInfo = Cache::get("account_info_" . md5($cookie));
        if ($accountInfo) return ResponseController::success($accountInfo);

        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent"),
                "Cookie" => $cookie
            ],
            "query" => [
                "method" => "uinfo"
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::getAccountInfo",
            "get",
            "https://pan.baidu.com/rest/2.0/xpan/nas",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("getAccountInfo error", [$options, $responseData]);
            return ResponseController::getAccountInfoFailed($responseData["errno"] ?? "未知", $responseData["errmsg"] ?? "未知");
        }

        $accountInfo = collect($responseData)->only(["avatar_url", "baidu_name", "netdisk_name", "uk", "vip_type"])->toArray();
        Cache::set("account_info_" . md5($cookie), $accountInfo, 60 * 60 * 24);
        return ResponseController::success($accountInfo);
    }

    public static function getEnterpriseInfo($cookie)
    {
        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent"),
                "Cookie" => $cookie
            ],
            "query" => [
                "clienttype" => "0",
                "app_id" => self::EnterpriseAppId
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::getEnterpriseInfo",
            "get",
            "https://pan.baidu.com/mid_enterprise_v2/api/enterprise/organization/allorganizationinfo",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("getEnterpriseInfo error", [$options, $responseData]);
            return ResponseController::getEnterpriseInfoFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知");
        }

        $orgInfo = collect($responseData["data"])
            ->map(fn($item) => [
                "cid" => $item["cid"],
                "org_name" => $item["orgInfo"]["name"],
                "product_name" => $item["orgInfo"]["product_name"],
                "product_starttime" => $item["orgInfo"]["product_starttime"],
                "product_endtime" => $item["orgInfo"]["product_endtime"],
            ]);

        return ResponseController::success($orgInfo);
    }

    public static function getFileList($surl, $pwd = "", $dir = "/", $page = 1, $num = 1000, $order = "filename")
    {
        if (is_numeric(substr($surl, 0, 1))) $surl = substr($surl, 1);
        $surl = "1" . $surl;

        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_wx_user_agent"),
                "Cookie" => config("BDWP.fake_cookie")
            ],
            "query" => [
                "channel" => "weixin",
                "version" => "2.9.6",
                "clienttype" => 25,
                "web" => 1,
                "qq-pf-to" => "pcqq.c2c"
            ],
            "form_params" => [
                "shorturl" => $surl,
                "pwd" => $pwd,
                "dir" => $dir,
                "root" => $dir === "/" ? 1 : 0,
                "page" => $page,
                "num" => min($num, 100),
                "order" => $order
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::getFileList",
            "post",
            "https://pan.baidu.com/share/wxlist",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            !isset($responseData["errtype"])
        ) {
            Log::error("getFileList error", [$options, $responseData]);
            return ResponseController::getFileListFailed($responseData["errno"] ?? "未知", $responseData["errtype"] ?? "未知");
        }

        $errno = $responseData["errno"];
        $errtype = $responseData["errtype"];

        if ($errno !== 0) {
            Log::error("getFileList error", [$options, $responseData]);

            if ($errno == "-6") return ResponseController::canNotUsePrivateLink();

            $info = match ($errtype) {
                0 => "啊哦，你来晚了，分享的文件已经被删除了，下次要早点哟。",
                1 => "啊哦，你来晚了，分享的文件已经被取消了，下次要早点哟。",
                2 => "此链接分享内容暂时不可访问",
                3 => "此链接分享内容可能因为涉及侵权、色情、反动、低俗等信息，无法访问！",
                5 => "啊哦！链接错误没找到文件，请打开正确的分享链接!",
                10 => "啊哦，来晚了，该分享文件已过期",
                11 => "由于访问次数过多，该分享链接已失效",
                12 => "因该分享含有自动备份文件夹，暂无法查看",
                15 => "系统升级，链接暂时无法查看，升级完成后恢复正常。",
                17 => "该链接访问范围受限，请使用正常的访问方式",
                123 => "该链接已超过访问人数上限，可联系分享者重新分享",
                124 => "您访问的链接已被冻结，可联系分享者进行激活",
                "mis_105" => "surl 错误",
                "mispw_9", "mispwd-9" => "提取码错误",
                "mis_2", "mis_4" => "路径错误",
                default => $errtype
            };

            return ResponseController::getFileListFailed($errno, $info);
        }

        $fileListData = $responseData["data"];
        return ResponseController::success([
            "uk" => $fileListData["uk"],
            "shareid" => $fileListData["shareid"],
            "randsk" => str_replace(["-", "~", "_"], ["+", "=", "/"], $fileListData["seckey"]),
            "ctime" => $fileListData["link_ctime"],
            "list" => collect($fileListData["list"])
                ->map(fn($item) => [
                    "category" => (int)$item["category"],
                    "fs_id" => (double)$item["fs_id"],
                    "is_dir" => ((int)$item["isdir"]) === 1,
                    "local_ctime" => (double)$item["local_ctime"],
                    "local_mtime" => (double)$item["local_mtime"],
                    "md5" => $item["md5"] ?? "",
                    "path" => $item["path"],
                    "server_ctime" => (double)$item["server_ctime"],
                    "server_mtime" => (double)$item["server_mtime"],
                    "server_filename" => $item["server_filename"],
                    "size" => (double)$item["size"],
                    "dlink" => $item["dlink"] ?? ""
                ]),
        ]);
    }

    public static function getTemplateVariable($cookie, $fields = ["bdstoken", "uk"])
    {
        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent"),
                "Cookie" => $cookie
            ],
            "query" => [
                "fields" => JSON::encode($fields)
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::getTemplateVariable",
            "get",
            "https://pan.baidu.com/api/gettemplatevariable",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            !isset($responseData["result"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("getTemplateVariable error", [$options, $responseData]);
            return ResponseController::getTemplateVariableFailed($responseData["errno"] ?? "未知");
        }

        return ResponseController::success($responseData["result"]);
    }

    private const DEFAULT_TICKET_LIST = [
        [
            "control_type" => 0,
            "expired_count" => 0,
            "expired_time" => 0,
            "package_count" => 0,
            "package_type" => 2,
            "recent_expired_time" => 0,
            "recent_remain_count" => 0,
            "recent_total_count" => 0,
            "recent_use_count" => 0,
            "remain_count" => 0,
            "start_time" => 0,
            "status" => 0,
            "total_count" => 0,
            "unit" => "张",
            "use_count" => 0,
            "value" => 0
        ]
    ];

    public static function getTicketList($cookie, $cid, $bdstoken, $package_type_list = null)
    {
        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent"),
                "Cookie" => $cookie
            ],
            "query" => [
                "clienttype" => 0,
                "app_id" => self::EnterpriseAppId,
                "web" => 1,
                "third_account_type" => 1,
                "cid" => $cid,
                "bdstoken" => $bdstoken
            ],
            "form_params" => [
                "tkappid" => 10000,
                ...($package_type_list ? ["package_type_list" => JSON::encode($package_type_list)] : []),
                "bdstoken" => $bdstoken
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::getTicketList",
            "post",
            "https://pan.baidu.com/mid_enterprise_v2/ticket/api/list",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("getTicketList error", [$options, $responseData]);
            return ResponseController::getTicketListFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知");
        }

        return ResponseController::success($responseData["data"]["package_list"] ?? self::DEFAULT_TICKET_LIST);
    }

    public static function getShareRecord($cookie, $cid)
    {
        $list = [];
        $page = 1;

        while (true) {
            $options = [
                "headers" => [
                    "User-Agent" => config("BDWP.fake_user_agent"),
                    "Cookie" => $cookie
                ],
                "query" => [
                    "channel" => "chunlei",
                    "clienttype" => "0",
                    "app_id" => "24029990",
                    "third_account_type" => "1",
                    "num" => "100",
                    "page" => $page,
                    "web" => "1",
                    "order" => "ctime",
                    "desc" => "1",
                    "cid" => $cid,
                    "is_batch" => "1"
                ]
            ];

            $response = UtilsController::sendRequest(
                "ApiController::getShareRecord",
                "post",
                "https://pan.baidu.com/mid_enterprise_v2/share/record",
                $options
            );

            $responseData = $response->getData(true);
            if ($responseData["code"] !== 200) return $response;

            $responseData = $responseData["data"];
            if (
                !isset($responseData["errno"]) ||
                $responseData["errno"] !== 0
            ) {
                Log::error("getShareRecord error", [$options, $responseData]);
                return ResponseController::getShareRecordFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知");
            }

            if ($responseData["count"] === 0) break;

            $page++;

            $newList = collect($responseData["list"])
                ->filter(fn($record) => !str_contains($record["typicalPath"], "分享的文件已被删除") && !str_contains($record["typicalPath"], "分享已过期"))
                ->values()
                ->toArray();

            $list = array_merge($list, $newList);
        }

        return ResponseController::success($list);
    }

    public static function getShareBindList($cookie, $cid, $shareid)
    {
        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent"),
                "Cookie" => $cookie
            ],
            "query" => [
                "tkappid" => "10000",
                "activity_id" => $shareid,
                "clienttype" => "21",
                "app_id" => "24029990",
                "cid" => $cid
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::getShareBindList",
            "get",
            "https://pan.baidu.com/mid_enterprise_v2/ticket/api/getbindlist",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("getShareBindList error", [$options, $responseData]);
            return ResponseController::getShareBindListFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知");
        }

        if ($responseData["data"]["tkbind_list"] === null) $responseData["data"]["tkbind_list"] = [];
        return ResponseController::success(collect($responseData["data"])->only(["tkbind_list", "use_count", "total_count"]));
    }

    public static function getFullCookie($cookie)
    {
        if (!str_ends_with($cookie, ";")) $cookie = $cookie . ";";

        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent"),
                "Cookie" => $cookie
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::getFullCookie",
            "get",
            "https://pan.baidu.com/disk/main",
            $options,
            true
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];

        $header = $responseData["header"];
        $setCookie = $header["Set-Cookie"];
        foreach ($setCookie as $item) {
            $newCookies = explode(";", $item);
            if (!$newCookies) continue;
            $cookie = $cookie . $newCookies[0] . ";";
        }

        return ResponseController::success([
            "cookie" => $cookie
        ]);
    }

    public static function ddddocr($type, $vcode_str)
    {
        $options = [
            "query" => [
                "type" => $type,
                "vcode_str" => $vcode_str
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::ddddocr",
            "post",
            "https://ddddocr.huankong.top",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (!isset($responseData["data"])) {
            Log::error("ddddocr error", [$options, $responseData]);
            return ResponseController::ddddocrServerFailed($responseData["message"] ?? "未知");
        }

        return ResponseController::success($responseData["data"]);
    }

    public static function checkPhoneStatus($phone)
    {
        $gid = self::makeGid();

        $phone2 = '';
        for ($i = 0; $i < 11; $i++) {
            $phone2 .= $phone[$i];
            if ($i == 2 || $i == 6) $phone2 .= '+';
        }

        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent")
            ],
            "query" => [
                "v" => time() . "0000"
            ],
            "form_params" => [
                'mobilenum' => $phone2,
                'clientfrom' => '',
                'tpl' => 'tb',
                'login_share_strategy' => '',
                'client' => '',
                'adapter' => '0',
                't' => time() . '0000',
                'act' => 'bind_mobile',
                'loginLink' => '0',
                'smsLoginLink' => '1',
                'lPFastRegLink' => '0',
                'fastRegLink' => '1',
                'lPlayout' => '0',
                'lang' => 'zh-cn',
                'regLink' => '1',
                'action' => 'login',
                'loginmerge' => '1',
                'isphone' => '0',
                'dialogVerifyCode' => '',
                'dialogVcodestr' => '',
                'dialogVcodesign' => '',
                'gid' => $gid,
                'agreement' => '1',
                'vcodesign' => '',
                'vcodestr' => '',
                'sms' => '1',
                'username' => $phone,
                'countrycode' => '',
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::checkPhoneStatus",
            "post",
            "https://wappass.baidu.com/wp/api/security/getphonestatus",
            $options,
            true
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["data"]["errInfo"]) ||
            $responseData["data"]["errInfo"]["no"] !== "0"
        ) {
            Log::error("checkPhoneStatus error", [$options, $responseData]);
            if ($responseData["data"]["errInfo"]["msg"] === "不存在") $responseData["data"]["errInfo"]["msg"] = "此手机号不存在或未开启手机账号登陆，请根据下方图片开启";
            return ResponseController::checkPhoneStatusFailed($responseData["data"]["errInfo"]["msg"] ?? "未知");
        }

        $setCookie = join("", $responseData["header"]["Set-Cookie"]);
        preg_match("/BAIDUID=(.*?);/", $setCookie, $BAIDUID);
        if (!$BAIDUID) return ResponseController::checkPhoneStatusFailed("获取BAIDUID失败");

        Cache::set("phone_login_{$BAIDUID[1]}", $gid, 60 * 60 * 24);

        return ResponseController::success([
            "BAIDUID" => $BAIDUID[1]
        ]);
    }

    public static function sendSms($phone, $BAIDUID)
    {
        $vcode = '';
        $vcodestr = '';
        $vcodesign = '';
        $gid = Cache::get("phone_login_{$BAIDUID}", "E528690-4ADF-47A5-BA87-1FD76D2583EA");

        $count = 0;
        while (true) {
            $options = [
                "headers" => [
                    "User-Agent" => config("BDWP.fake_user_agent"),
                    "Cookie" => 'BAIDUID=' . $BAIDUID . ';'
                ],
                "query" => [
                    "v" => time() . "0000"
                ],
                "form_params" => [
                    'username' => $phone,
                    'tpl' => "tb",
                    "clientfrom" => "",
                    "countrycode" => "",
                    "gid" => $gid,
                    "dialogVerifyCode" => $vcode,
                    "vcodesign" => $vcodesign,
                    "vcodestr" => $vcodestr,
                    "vstrswitch" => "1",
                    "time" => time() . "0000"
                ]
            ];

            $response = UtilsController::sendRequest(
                "ApiController::sendSms",
                "post",
                "https://wappass.baidu.com/wp/api/login/sms",
                $options,
            );

            $responseData = $response->getData(true);
            if ($responseData["code"] !== 200) return $response;

            $responseData = $responseData["data"];
            if (!isset($responseData["errInfo"])) {
                Log::error("sendSms error", [$options, $responseData]);
                return ResponseController::sendSmsFailed($responseData["errInfo"]["msg"] ?? "未知");
            }

            if ($responseData["errInfo"]["no"] === "50020") {
                if ($count === 5) return ResponseController::sendSmsFailed("发送验证码次数过多，请稍后再试");

                $vcode = $responseData['data']['vcodestr'];
                $vcodesign = $responseData['data']['vcodesign'];

                $ddddocr = self::ddddocr("login", $vcode);
                $ddddocrData = $ddddocr->getData(true);
                if ($ddddocrData["code"] !== 200) return $ddddocr;

                $ddddocrData = $ddddocrData["data"];
                $vcodestr = $ddddocrData["vcode_input"];

                $count++;
                continue;
            } else if ($responseData["errInfo"]["no"] !== "0") {
                Log::error("sendSms error", [$options, $responseData]);
                return ResponseController::sendSmsFailed($responseData["errInfo"]["msg"] ?? "未知");
            }

            return ResponseController::success();
        }
    }

    public static function loginBySms($phone, $BAIDUID, $smsvc)
    {
        $gid = Cache::get("phone_login_{$BAIDUID}", "E528690-4ADF-47A5-BA87-1FD76D2583EA");

        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent"),
                "Cookie" => 'BAIDUID=' . $BAIDUID . ';'
            ],
            "query" => [
                "v" => time() . "0000"
            ],
            "form_params" => [
                'smsvc' => $smsvc,
                'clientfrom' => '',
                'tpl' => 'tb',
                'login_share_strategy' => '',
                'client' => '',
                'adapter' => '0',
                't' => time() . '0000',
                'loginLink' => '0',
                'smsLoginLink' => '1',
                'lPFastRegLink' => '0',
                'fastRegLink' => '1',
                'lPlayout' => '0',
                'lang' => 'zh-cn',
                'regLink' => '1',
                'action' => 'login',
                'loginmerge' => '',
                'isphone' => '0',
                'dialogVerifyCode' => '',
                'dialogVcodestr' => '',
                'dialogVcodesign' => '',
                'gid' => $gid,
                'agreement' => '1',
                'vcodesign' => '',
                'vcodestr' => '',
                'smsverify' => '1',
                'sms' => '1',
                'mobilenum' => $phone,
                'username' => $phone,
                'countrycode' => '',
                'passAppHash' => '',
                'passAppVersion' => '',
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::loginBySms",
            "post",
            "https://wappass.baidu.com/wp/api/login",
            $options,
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errInfo"]) ||
            $responseData["errInfo"]["no"] !== "0"
        ) {
            Log::error("loginBySms error", [$options, $responseData]);
            return ResponseController::loginBySmsFailed($responseData["errInfo"]["msg"] ?? "未知");
        }

        $bduss = $responseData["data"]["bduss"];
        $xml = $responseData["data"]["xml"];
        preg_match("~<stoken>netdisk#(.*?)</stoken>~", $xml, $stoken);
        if (!$stoken) return ResponseController::loginBySmsFailed("获取stoken失败");
        $stoken = $stoken[1];

        return ResponseController::success([
            "bduss" => $bduss,
            "stoken" => $stoken
        ]);
    }

    public static function makeGid()
    {
        return strtoupper(
            preg_replace_callback(
                '/[xy]/',
                function ($matches) {
                    $r = mt_rand(0, 15);
                    $v = ($matches[0] === 'x') ? $r : ($r & 0x3 | 0x8);
                    return dechex($v);
                },
                'xxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'
            )
        );
    }

    public static function getQrCode()
    {
        $gid = self::makeGid();

        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent"),
            ],
            "query" => [
                "lp" => "pc",
                "qrloginfrom" => "pc",
                "gid" => $gid,
                "apiver" => "v3",
                "tt" => time() . "0000",
                "tpl" => "netdisk"
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::getQrCode",
            "get",
            "https://passport.baidu.com/v2/api/getqrcode",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("getQrCode error", [$options, $responseData]);
            return ResponseController::getQrCodeFailed();
        }

        $loginLink = "https://wappass.baidu.com/wp/?qrlogin&t=" . time() . "&error=0&sign=" . $responseData['sign'] . "&cmd=login&lp=pc&tpl=netdisk&adapter=3&qrloginfrom=pc&local=";
        if (!str_starts_with($responseData["imgurl"], "http")) $responseData["imgurl"] = "https://" . $responseData["imgurl"];

        Cache::set("qrlogin_{$responseData['sign']}", $gid, 60 * 60 * 24);

        return ResponseController::success([
            "img_url" => $responseData["imgurl"],
            "sign" => $responseData['sign'],
            "link" => $loginLink
        ]);
    }

    public static function checkStatus($sign)
    {
        $gid = Cache::get("qrlogin_{$sign}", "07D9D20-91EB-43D8-8553-16A98A0B24AA");

        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent"),
            ],
            "query" => [
                "channel_id" => $sign,
                "gid" => $gid,
                "tpl" => "netdisk",
                "apiver" => "v3",
                "tt" => time() . "0000"
            ],
            "timeout" => Str::contains(url()->current(), ["localhost", "127.0.0.1"]) ? 5 : 60
        ];

        $response = UtilsController::sendRequest(
            "ApiController::checkStatus",
            "get",
            "https://passport.baidu.com/channel/unicast",
            $options,
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            isset($responseData["errno"]) &&
            $responseData["errno"] === 1
        ) {
            return ResponseController::networkError("ApiController::checkStatus");
        }

        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("checkStatus error", [$options, $responseData]);
            return ResponseController::getQrCodeStatusFailed();
        }

        $channel_v = JSON::decode($responseData["channel_v"]);
        if (!$channel_v) return ResponseController::getQrCodeStatusFailed();

        if ($channel_v["status"] === 1) {
            return ResponseController::qrCodeScanned();
        } else if ($channel_v["status"] === 2) {
            return ResponseController::qrCodeScanCanceled();
        } else if ($channel_v["status"] === 0) {
            // 成功
            return ResponseController::success([
                "bduss" => $channel_v["v"]
            ]);
        } else {
            return ResponseController::getQrCodeStatusFailed();
        }
    }

    public static function loginByBduss($bduss)
    {
        $options = [
            "headers" => [
                "User-Agent" => config("BDWP.fake_user_agent"),
                "Cookie" => "BDUSS={$bduss};"
            ],
            "query" => [
                "v" => time() . "0000",
                "bduss" => $bduss,
                "u" => "https://passport.baidu.com/",
                "loginVersion" => "v5",
                "qrcode" => "1",
                "tpl" => "netdisk",
                "maskId" => "",
                "fileId" => "",
                "apiver" => "v3",
                "tt" => time() . "0000"
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::loginByBduss",
            "get",
            "https://passport.baidu.com/v3/login/main/qrbdusslogin",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errInfo"]) ||
            $responseData["errInfo"]["no"] !== "0"
        ) {
            Log::error("loginByBduss error", [$options, $responseData]);
            return ResponseController::loginByBdussFailed($loginByBdussData["errInfo"]["msg"] ?? "未知");
        }

        $netdiskStoken = collect($responseData["data"]["session"]["stokenList"])->filter(fn($item) => str_starts_with($item, "netdisk#"));
        if ($netdiskStoken->isEmpty()) return ResponseController::loginByBdussFailed("获取stoken失败");
        $netdiskStoken = $netdiskStoken->first();

        return ResponseController::success([
            "bduss" => $responseData["data"]["session"]["bduss"],
            "stoken" => explode("netdisk#", $netdiskStoken)[1]
        ]);
    }

    const devuid = "0";

    const PCUa = "netdisk;4.37.5;PC;PC-Windows;10.0.26100;UniBaiduYunGuanJia";
    const PCChannel = "Windows_10.0.26100_Uniyunguanjia_netdisk_00000000000000000000000040000001";
    const PCVersion = "4.37.5";

    const UnionVersion = "7.51.0.133";
    const DownloadEngineVersion = "3.0.20.88";

    const AndroidUa = "netdisk;12.16.54;23049RAD8C;android-android;13;JSbridge4.4.0;jointBridge;1.1.0;";
    const AndroidChannel = "android_13_23049RAD8C_bd-netdisk_1027840c";
    const AndroidVersion = "12.16.54";

    const NormalAppId = "250528";
    const EnterpriseAppId = "24029990";

    public static function enterpriseCreateFolder($cookie, $cid, $path)
    {
        $options = [
            "headers" => [
                "User-Agent" => self::PCUa,
                "Cookie" => $cookie
            ],
            "query" => [
                "cid" => $cid,
                "app_id" => self::EnterpriseAppId,
                "clienttype" => "8",
                "channel" => self::PCChannel,
                "version" => self::PCVersion,
                "devuid" => self::devuid,
                "vip" => "0",
                "async" => "0"
            ],
            "form_params" => [
                "path" => $path,
                "size" => "0",
                "isdir" => "1"
            ],
        ];

        $response = UtilsController::sendRequest(
            "ApiController::enterpriseCreateFolder",
            "post",
            "https://pan.baidu.com/mid_enterprise_v2/api/create",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("enterpriseCreateFolder error", [$options, $responseData]);
            if ($responseData["show_msg"] === "other async job is doing now") $responseData["show_msg"] = "当前有其他异步任务正在进行，请稍后再试";
            return ResponseController::createFolderFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知");
        }

        return ResponseController::success();
    }

    public static function enterpriseCreateShareLink($cookie, $bdstoken, $cid, $path, $pwd)
    {
        // 输出堆栈信息到日志
        Log::debug("enterpriseCreateShareLink called", [
            "cookie" => $cookie,
            "bdstoken" => $bdstoken,
            "cid" => $cid,
            "path" => $path,
            "pwd" => $pwd,
            "trace" => (new Exception())->getTraceAsString()
        ]);

        $options = [
            "headers" => [
                "User-Agent" => self::PCUa,
                "Cookie" => $cookie
            ],
            "query" => [
                "cid" => $cid,
                "app_id" => self::EnterpriseAppId,
                "clienttype" => "8",
                "channel" => self::PCChannel,
                "version" => self::PCVersion,
                "devuid" => self::devuid,
            ],
            "form_params" => [
                "period" => "0",
                "watermark" => "",
                "isAddWatermark" => "false",
                "truncate" => "0",
                "pwd" => $pwd,
                ...(config("BDWP.admin.do_not_use_download_ticket")
                    ? []
                    : ["ticket" => JSON::encode(["package_type" => 2, "count" => 1])]
                ),
                "channel_list" => "[]",
                "schannel" => "4",
                "path_list" => JSON::encode([$path]),
            ],
        ];

        $response = UtilsController::sendRequest(
            "ApiController::enterpriseCreateShareLink",
            "post",
            "https://pan.baidu.com/mid_enterprise_v2/share/set",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("enterpriseCreateShareLink error", [$options, $responseData]);
            return ResponseController::createShareLinkFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知");
        }

        return ResponseController::success($responseData);
    }

    public static function enterpriseTransfer($cookie, $cid, $shareid, $uk, $randsk, $fsidlist, $path, $_task_id = null, $index = null, $count = null)
    {
        $options = [
            "headers" => [
                "User-Agent" => self::AndroidUa,
                "Cookie" => $cookie
            ],
            "query" => [
                "shareid" => $shareid,
                "from" => $uk,
                "sekey" => $randsk,
                "app_id" => self::EnterpriseAppId,
                "ondup" => "newcopy",
                "async" => "1",
                "cid" => $cid,
                "channel" => self::AndroidChannel,
                "devuid" => self::devuid,
                "clienttype" => "1",
                "version" => self::AndroidVersion
            ],
            "form_params" => [
                "path" => $path,
                "fsidlist" => JSON::encode($fsidlist),
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::enterpriseTransfer",
            "post",
            "https://pan.baidu.com/mid_enterprise_v2/share/transfer",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("enterpriseTransfer error", [$options, $responseData]);
            if (isset($responseData["target_file_nums"]) && isset($responseData["target_file_nums_limit"])) {
                return ResponseController::transferFileFailed(
                    $responseData["errno"] ?? "未知",
                    ($responseData["show_msg"] ?? "未知") . ", 本次转存了 {$responseData['target_file_nums']} 个文件, 只能转存 {$responseData['target_file_nums_limit']} 个文件"
                );
            }
            return ResponseController::transferFileFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知");
        }

        if ($responseData["show_msg"] === "文件转存中" || $responseData["task_id"] !== 0) {
            while (true) {
                $taskQuery = self::enterpriseTaskQuery(
                    $cookie,
                    $cid,
                    $responseData["task_id"],
                    $_task_id,
                    $index,
                    $count
                );
                $taskQueryData = $taskQuery->getData(true);

                if ($taskQueryData["code"] !== 200) {
                    if (str_contains($taskQueryData["message"], "任务进行中")) {
                        sleep(3);
                        continue;
                    }

                    return $taskQuery;
                }

                return ResponseController::success(["list" => $taskQueryData["data"]["list"]]);
            }
        }

        return ResponseController::success(["list" => $responseData["extra"]["list"]]);
    }

    public static function transfer($cookie, $shareid, $uk, $randsk, $fsidlist, $path, $task_id = null, $index = null, $count = null)
    {
        $options = [
            "headers" => [
                "User-Agent" => self::PCUa,
                "Cookie" => $cookie
            ],
            "query" => [
                "clienttype" => "8",
                "channel" => "00000000000000000000000040000001",
                "version" => self::PCVersion,
                "devuid" => self::devuid,
                "shareid" => $shareid,
                "from" => $uk,
                "ondup" => "newcopy",
                "force" => "0",
                "async" => "1"
            ],
            "form_params" => [
                "path" => $path,
                "fsidlist" => JSON::encode($fsidlist),
                "sekey" => $randsk,
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::transfer",
            "post",
            "https://pan.baidu.com/share/transfer",
            $options
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("transfer error", [$options, $responseData]);
            // 获取用户盘内信息
            $quota = ApiController::getQuota($cookie);
            if (isset($responseData["target_file_nums"]) && isset($responseData["target_file_nums_limit"])) {
                return ResponseController::transferFileFailed(
                    $responseData["errno"] ?? "未知",
                    ($responseData["show_msg"] ?? "未知") . ", 本次转存了 {$responseData['target_file_nums']} 个文件, 只能转存 {$responseData['target_file_nums_limit']} 个文件",
                    false,
                    [
                        "target_file_nums" => $responseData["target_file_nums"],
                        "target_file_nums_limit" => $responseData["target_file_nums_limit"],
                    ]
                );
            }
            return ResponseController::transferFileFailed(
                $responseData["errno"] ?? "未知",
                $responseData["show_msg"] ?? "未知",
                false,
                $quota->getData(true)["data"]
            );
        }

        if ($responseData["show_msg"] === "文件转存中" || $responseData["task_id"] !== 0) {
            while (true) {
                $taskQuery = self::taskQuery(
                    $cookie,
                    $responseData["task_id"],
                    $task_id,
                    $index,
                    $count
                );
                $taskQueryData = $taskQuery->getData(true);

                if ($taskQueryData["code"] !== 200) {
                    if (str_contains($taskQueryData["message"], "任务进行中")) {
                        sleep(3);
                        continue;
                    }

                    if (str_contains($taskQueryData["message"], "-32")) {
                        // 获取用户盘内信息
                        $quota = ApiController::getQuota($cookie);
                        return ResponseController::transferFileFailed(
                            "-32",
                            "剩余空间不足",
                            false,
                            $quota->getData(true)["data"]
                        );
                    }

                    return $taskQuery;
                }

                return ResponseController::success(["list" => $taskQueryData["data"]["list"]]);
            }
        }

        return ResponseController::success(["list" => $responseData["extra"]["list"]]);
    }

    public static function enterpriseTaskQuery($cookie, $cid, $task_id, $_task_id = null, $file_index = null, $file_count = null)
    {
        $count = 0;
        while (true) {
            $options = [
                "headers" => [
                    "User-Agent" => self::AndroidUa,
                    "Cookie" => $cookie
                ],
                "query" => [
                    "taskid" => $task_id,
                    "cid" => $cid,
                    "app_id" => "24029990",
                    "channel" => "chunlei",
                    "web" => 1,
                    "clienttype" => "0"
                ]
            ];

            $response = UtilsController::sendRequest(
                "ApiController::enterpriseTaskQuery",
                "GET",
                "https://pan.baidu.com/mid_enterprise_v2/share/taskquery",
                $options
            );

            $responseData = $response->getData(true);
            if ($responseData["code"] !== 200) {
                Log::error("enterpriseTaskQuery error", [$options, $responseData]);

                $count++;
                if ($count >= 5) return $response;
                continue;
            }
            $responseData = $responseData["data"];

            if ($_task_id !== null && $file_index !== null && $file_count !== null && isset($responseData["progress"])) {
                if ($file_index <= 0 || $file_count <= 0) {
                    $overallProgress = $responseData["progress"];
                } else {
                    $overallProgress = (($file_index - 1) * 100 + $responseData["progress"]) / $file_count;
                }

                // 可以四舍五入保留整数百分比
                $overallProgress = round($overallProgress, 2);

                $status = ResponseController::working($overallProgress);
                AsyncTask::setCache($_task_id, $status);
            }

            if ($responseData["status"] === "success") return ResponseController::success($responseData);
            if ($responseData["status"] === "running") return ResponseController::taskWorking();
            return ResponseController::taskQueryError(true, $responseData["task_errno"] ?? "未知");
        }
    }

    public static function taskQuery($cookie, $task_id, $_task_id = null, $file_index = null, $file_count = null)
    {
        $count = 0;
        while (true) {
            $options = [
                "headers" => [
                    "User-Agent" => self::PCUa,
                    "Cookie" => $cookie
                ],
                "query" => [
                    "taskid" => $task_id,
                    "app_id" => "270528",
                    "channel" => "chunlei",
                    "web" => 1,
                    "clienttype" => "0"
                ]
            ];

            $response = UtilsController::sendRequest(
                "ApiController::enterpriseTaskQuery",
                "GET",
                "https://pan.baidu.com/share/taskquery",
                $options
            );

            $responseData = $response->getData(true);
            if ($responseData["code"] !== 200) {
                Log::error("taskQuery error", [$options, $responseData]);

                $count++;
                if ($count >= 5) return $response;
                continue;
            }
            $responseData = $responseData["data"];

            if ($_task_id !== null && $file_index !== null && $file_count !== null && isset($responseData["progress"])) {
                if ($file_index <= 0 || $file_count <= 0) {
                    $overallProgress = $responseData["progress"];
                } else {
                    $overallProgress = (($file_index - 1) * 100 + $responseData["progress"]) / $file_count;
                }

                // 可以四舍五入保留整数百分比
                $overallProgress = round($overallProgress, 2);

                $status = ResponseController::working($overallProgress);
                AsyncTask::setCache($_task_id, $status);
            }

            if ($responseData["status"] === "success") return ResponseController::success($responseData);
            if ($responseData["status"] === "running") return ResponseController::taskWorking();
            return ResponseController::taskQueryError(false, $responseData["task_errno"] ?? "未知");
        }
    }

    public static function enterpriseDeleteFolder($cookie, $cid, $filelist)
    {
        $option = [
            "headers" => [
                "User-Agent" => self::PCUa,
                "Cookie" => $cookie
            ],
            "query" => [
                "async" => "2",
                "opera" => "delete",
                "cid" => $cid,
                "app_id" => self::EnterpriseAppId,
                "clienttype" => "8",
                "channel" => self::PCChannel,
                "version" => self::PCVersion,
                "devuid" => self::devuid,
                "vip" => "0"
            ],
            "form_params" => [
                "filelist" => JSON::encode($filelist)
            ],
        ];

        $response = UtilsController::sendRequest(
            "ApiController::enterpriseDeleteFolder",
            "post",
            "https://pan.baidu.com/mid_enterprise_v2/api/filemanager",
            $option
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("enterpriseDeleteFolder error", [$option, $responseData]);
            return ResponseController::deleteFolderFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知");
        }

        if (isset($responseData["task_id"]) && $responseData["task_id"] !== 0) {
            while (true) {
                $taskQuery = self::enterpriseTaskQuery($cookie, $cid, $responseData["task_id"]);
                $taskQueryData = $taskQuery->getData(true);
                if ($taskQueryData["code"] !== 200) {
                    if (str_contains($taskQueryData["message"], "任务进行中")) {
                        sleep(3);
                        continue;
                    }

                    return $taskQuery;
                }

                break;
            }
        }

        return ResponseController::success();
    }

    public static function getList($cookie, $path)
    {
        $option = [
            "headers" => [
                "User-Agent" => self::PCUa,
                "Cookie" => $cookie
            ],
            "query" => [
                "clienttype" => "0",
                "app_id" => "250528",
                "web" => "1",
                "order" => "time",
                "desc" => "1",
                "dir" => $path,
                "num" => "100",
                "page" => "1"
            ],
        ];

        $response = UtilsController::sendRequest(
            "ApiController::getList",
            "get",
            "https://pan.baidu.com/api/list",
            $option
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("getList error", [$option, $responseData]);
            return ResponseController::getListFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知", false);
        }

        return ResponseController::success($responseData["list"]);
    }

    public static function createFolder($cookie, $path)
    {
        $option = [
            "headers" => [
                "User-Agent" => self::PCUa,
                "Cookie" => $cookie
            ],
            "query" => [
                "clienttype" => "8",
                "channel" => "00000000000000000000000040000001",
                "version" => self::PCVersion
            ],
            "form_params" => [
                "path" => $path,
                "isdir" => "1",
                'block_list' => [],
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::createFolder",
            "post",
            "https://pan.baidu.com/api/create",
            $option
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("createFolder error", [$option, $responseData]);
            return ResponseController::createFolderFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知", false);
        }

        return ResponseController::success();
    }

    public static function getQuota($cookie)
    {
        $option = [
            "headers" => [
                "User-Agent" => self::PCUa,
                "Cookie" => $cookie
            ],
            "query" => []
        ];

        $response = UtilsController::sendRequest(
            "ApiController::getQuota",
            "post",
            "https://pan.baidu.com/api/quota",
            $option
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("getQuota error", [$option, $responseData]);
            return ResponseController::getQuotaFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知", false);
        }

        return ResponseController::success([
            "used" => $responseData["used"],
            "total" => $responseData["total"],
        ]);
    }

    public static function getEnterpriseList($cookie, $cid, $path, $all = false)
    {
        $list = [];
        $page = 1;

        while (true) {
            $option = [
                "headers" => [
                    "User-Agent" => self::PCUa,
                    "Cookie" => $cookie
                ],
                "query" => [
                    "clienttype" => "0",
                    "app_id" => "24029990",
                    "web" => "1",
                    "order" => "time",
                    "desc" => "1",
                    "dir" => $path,
                    "num" => "100",
                    "page" => $page,
                    "cid" => $cid
                ],
            ];

            $response = UtilsController::sendRequest(
                "ApiController::getEnterpriseList",
                "get",
                "https://pan.baidu.com/mid_enterprise_v2/api/list",
                $option
            );

            $responseData = $response->getData(true);
            if ($responseData["code"] !== 200) return $response;

            $responseData = $responseData["data"];
            if (
                !isset($responseData["errno"]) ||
                $responseData["errno"] !== 0
            ) {
                Log::error("getEnterpriseList error", [$option, $responseData]);
                return ResponseController::getListFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知");
            }

            $list = array_merge($list, $responseData["list"]);
            if (!$all) break;

            if (!$responseData["has_more"]) break;
            $page++;
        }

        return ResponseController::success($list);
    }

    public static function renameFile($cookie, $filelist)
    {
        $option = [
            "headers" => [
                "User-Agent" => self::PCUa,
                "Cookie" => $cookie
            ],
            "query" => [
                "async" => "2",
                "onnest" => "fail",
                "opera" => "rename",
                "clienttype" => "8",
                "channel" => self::PCChannel,
                "version" => self::PCVersion,
                "devuid" => self::devuid,
                "vip" => "0",
                "app_id" => self::NormalAppId
            ],
            "form_params" => [
                "filelist" => JSON::encode($filelist),
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::renameFile",
            "post",
            "https://pan.baidu.com/api/filemanager",
            $option
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("renameFile error", [$option, $responseData]);
            return ResponseController::renameFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知", false);
        }

        return ResponseController::success();
    }

    public static function getQrLoginToken($cookie, $sign)
    {
        $option = [
            "headers" => [
                "User-Agent" => "Mozilla/5.0 (Linux; Android 12; 2410CRP4CC Build/V417IR; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/101.0.4951.61 Mobile Safari/537.36 Sapi_9.10.4_Android_%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98_12.11.9_2410CRP4CC_12_Sapi",
                "Cookie" => $cookie
            ]
        ];

        $requestUrl = "https://wappass.baidu.com/wp/?qrlogin&error=0&cmd=login&lp=pc&tpl=netdisk&adapter=3&clientfrom=native&qrloginfrom=native&extrajson=&credentialKey=1&deliverParams=1&liveAbility=1&support_photo=1&suppcheck=1&supFaceLogin=1&scanface=1&sign={$sign}";

        $response = UtilsController::sendRequest(
            "ApiController::getQrLoginToken",
            "get",
            $requestUrl,
            $option
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];

        $key = "token";
        $tokenValue = null;

        // 1. 优先尝试从 tplVars 中获取
        if (preg_match('/tplVars\s*=\s*"([^"]+)"/', $responseData, $matches)) {
            try {
                $tplVarsStr = str_replace('\x22', '"', $matches[1]);
                $tplVarsData = json_decode($tplVarsStr, true);
                if ($tplVarsData && isset($tplVarsData[$key])) $tokenValue = $tplVarsData[$key];
            } catch (Exception $e) {
            }
        }

        // 2. 如果tplVars中没有找到，尝试正则匹配其他格式
        $patterns = [
            "/{$key}\s*[:=]\s*['\"]([^'\"]+)['\"]/",  // token:'xxx' 或 token="xxx"
            "/{$key}\s*=\s*['\"]([^'\"]+)['\"]/",     // token='xxx'
            "/['\"]?{$key}['\"]?\s*:\s*['\"]([^'\"]+)['\"]/",  // "token":"xxx"
            "/data-{$key}\s*=\s*['\"]([^'\"]+)['\"]/",  // data-token="xxx"
            "/{$key}\s*=\s*([^,\s]+)/"  // token=xxx
        ];

        foreach ($patterns as $pattern) {
            if (preg_match($pattern, $responseData, $matches)) {
                $tokenValue = $matches[1];
                break;
            }
        }

        if (!$tokenValue) {
            Log::error("getQrLoginToken error", [$option, $requestUrl, $responseData]);
            return ResponseController::getQrLoginTokenFailed();
        }

        return ResponseController::success([
            "token" => $tokenValue
        ]);
    }

    public static function doQrLogin($cookie, $sign, $token)
    {
        $option = [
            "headers" => [
                "User-Agent" => "Mozilla/5.0 (Linux; Android 12; 2410CRP4CC Build/V417IR; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/101.0.4951.61 Mobile Safari/537.36 Sapi_9.10.4_Android_%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98_12.11.9_2410CRP4CC_12_Sapi",
                "Cookie" => $cookie,
                "Origin" => "https://wappass.baidu.com",
                "Referer" => "https://wappass.baidu.com/wp/?qrlogin&sign={$sign}&v=" . time() . "0000"
            ],
            "form_params" => [
                "tpl" => "netdisk",
                "lp" => "pc",
                "token" => $token,
                "sign" => $sign,
            ]
        ];

        $requestUrl = "https://wappass.baidu.com/wp/?qrlogin";
        $requestUrl .= "&v=" . time() . "0000";

        $response = UtilsController::sendRequest(
            "ApiController::doQrLogin",
            "post",
            $requestUrl,
            $option
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData['errInfo']['no']) ||
            $responseData['errInfo']['no'] !== "0"
        ) {
            Log::error("doQrLogin error", [$option, $requestUrl, $responseData]);
            return ResponseController::doQrLoginFailed($responseData['errInfo']['no'] ?? "未知", $responseData['errInfo']['msg'] ?? "未知");
        }

        return ResponseController::success();
    }

    public static function enterpriseGetDirSize($cookie, $cid, $list)
    {
        $option = [
            "headers" => [
                "User-Agent" => self::PCUa,
                "Cookie" => $cookie
            ],
            "query" => [
                "clienttype" => "0",
                "app_id" => self::EnterpriseAppId,
                "web" => "1",
                "async" => "2",
                "list" => JSON::encode($list),
                "cid" => $cid
            ]
        ];

        $response = UtilsController::sendRequest(
            "ApiController::enterpriseGetDirSize",
            "get",
            "https://pan.baidu.com/mid_enterprise_v2/api/dirsize",
            $option
        );

        $responseData = $response->getData(true);
        if ($responseData["code"] !== 200) return $response;

        $responseData = $responseData["data"];
        if (
            !isset($responseData["errno"]) ||
            $responseData["errno"] !== 0
        ) {
            Log::error("enterpriseGetDirSize error", [$option, $responseData]);
            return ResponseController::enterpriseGetDirSizeFailed($responseData["errno"] ?? "未知", $responseData["show_msg"] ?? "未知");
        }

        while (true) {
            $taskQuery = self::enterpriseTaskQuery($cookie, $cid, $responseData["taskid"]);
            $taskQueryData = $taskQuery->getData(true);
            if ($taskQueryData["code"] !== 200) {
                if (str_contains($taskQueryData["message"], "任务进行中")) {
                    sleep(3);
                    continue;
                }

                return $taskQuery;
            }

            return ResponseController::success($taskQueryData["data"]);
        }
    }

    public static function testProxy()
    {
        return UtilsController::sendRequest(
            "ApiController::testProxy",
            "get",
            "https://www.cz88.net/api/cz88/ip/base?ip="
        );
    }
}
